...
class VerifyTotpVerificationRequest(BaseModel):
    """
    VerifyTotpVerificationRequest
    """ # noqa: E501
    code: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The TOTP code to be verified.")
    verification_id: Optional[StrictStr] = Field(default=None, description="The verification ID of the newly created TOTP secret. This ID is required to verify a newly created TOTP secret that needs to be bound to the user account. If not provided, the API will create a new TOTP verification record and verify the code against the user's existing TOTP secret.", alias="verificationId")
    __properties: ClassVar[List[str]] = ["code", "verificationId"]
