...
class GetUserAssetServiceStatus200Response(BaseModel):
    """
    ...
    def allow_upload_mime_types_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['image/jpeg', 'image/png', 'image/gif', 'image/vnd.microsoft.icon', 'image/x-icon', 'image/svg+xml', 'image/tiff', 'image/webp', 'image/bmp', 'application/zip']):
                raise ValueError("each list item must be one of ('image/jpeg', 'image/png', 'image/gif', 'image/vnd.microsoft.icon', 'image/x-icon', 'image/svg+xml', 'image/tiff', 'image/webp', 'image/bmp', 'application/zip')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "status": GetUserAssetServiceStatus200ResponseStatus.from_dict(obj["status"]) if obj.get("status") is not None else None,
